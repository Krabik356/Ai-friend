...
async def ai_response(user_data, audio, messages, conn, user_id):
...
async def stop(user_id=Depends(is_valid_id), conn=Depends(get_conn)):
    audio = audios[user_id]

    messages = []
    user_data = []
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT id, user_req, ai_resp_text FROM messages WHERE user_id=%s ORDER BY id DESC LIMIT 100", (user_id,))
        messages = await cur.fetchall()

        await cur.execute("SELECT reputation FROM users WHERE id=%s ORDER BY id ASC", (user_id,))
        user_data = await cur.fetchone()


    return StreamingResponse(
        ai_response(user_data, audio, messages, conn, user_id),
